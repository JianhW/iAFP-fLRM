...
def load_Ind_data(file):#独立测试集
    ...
    data_iter = list(data_iter)
    return data_iter
def load_bench_data_trans(file):#训练集
    tmp = pd.read_csv(file, header=None)
    seqs, labels = tmp[0].values.tolist(), tmp[1].values.tolist()
    data_iter = transformer_embedding(seqs, labels, file,train=True)
    data_iter = list(data_iter)
    return data_iter

def transformer_embedding(seqs, labels, file,train):
    # 加载预训练模型和分词器
    model_name = "bert-base-uncased"  # 可选：roberta-base, distilbert-base-uncased等
...
def data_construct(seqs, labels,file, train):#将氨基酸序列转化为模型所需要的输入
    ...
    HF_feature = HF_encoding(seqs, sequences,labels).detach().numpy()  # 确保返回的是NumPy数组
    dataset = Data.TensorDataset(embed_data, torch.FloatTensor(pos_embed),torch.FloatTensor(HF_feature),torch.FloatTensor([item.cpu().detach().numpy() for item in trans_codes]).cuda(), torch.LongTensor(labels))
    #print(dataset.shape)
    batch_size = 128
    data_iter = torch.utils.data.DataLoader(dataset, batch_size=batch_size, shuffle=train)
    return data_iter
...
class Net(nn.Module):
    def __init__(self,num_classes=2):
        ...
        self.trans_linearlayer=nn.Linear(768,128,device=device)
        self.relu=nn.ReLU()
    ...
    def forward(self, x, pos_embed,HF,trans):
        ...
        #print(output1.size())
        #print(HF.size())
        output2 = HF.unsqueeze(2)  # 假设 HF 形状为 (batch, seq_len), 添加特征维度 -> (batch, seq_len, 1)
        #print(output2.size())
...
def evaluate(data_iter, net,epoch):
    ...
    rep_list = []
    true_labels = []
    for x, pos,hf ,trans, y in data_iter:
        outputs,rep = net(x, pos, hf,trans)
        #print(rep.shape)
...
def train_test(train_iter, test_iter):
    ...
    for epoch in range(EPOCH):
        ...
        t0 = time.time()
        net.train()
        for seq, pos, hf,trans, label in train_iter:
            output,_,trans_output = net(seq, pos, hf,trans)
            loss = reg_loss(net, output, label)
            trans_loss = reg_loss(net,trans_output,label)
            optimizer.zero_grad()
            loss.backward()
            trans_loss.backward()
            optimizer.step()
        ...
        if (epoch+1)%50==0:
            torch.save(net.state_dict(),"net_{}.pth".format(epoch+1))
    #writer.close()
