...
class FeatureCalibrationModule(nn.Module):
    def __init__(self, embed_dim=128):
        super().__init__()
        # 线性变换：将BLOSUM特征（256维）降维到128维，匹配嵌入/位置编码维度
        self.linear_blosum = nn.Linear(256, embed_dim).to(device)
        # 自注意力层：输入为3个128维特征拼接（384维）
        self.self_attn = nn.MultiheadAttention(
            embed_dim=3 * embed_dim,  # 128*3=384
            num_heads=6,  # 384/6=64，可整除
            batch_first=True,
            dropout=0.0  # 先关闭dropout，稳定训练
        ).to(device)
        self.norm = nn.LayerNorm(3 * embed_dim).to(device)
        # 注意力权重缩放：先弱化影响
        self.attn_scale = nn.Parameter(torch.ones(1) * 0.1)

    def forward(self, embed, pos, blosum):
        # 1. BLOSUM特征降维到128维
        blosum = self.linear_blosum(blosum)  # (batch, seq_len, 128)
        # 2. 拼接3个128维特征 → 384维
        concat_feat = torch.cat([embed, pos, blosum], dim=-1)  # (batch, seq_len, 384)
        # 3. 自注意力加权（弱化）
        attn_feat, attn_weights = self.self_attn(concat_feat, concat_feat, concat_feat)
        attn_feat = attn_feat * self.attn_scale
        # 4. 残差连接+归一化
        calibrated_feat = self.norm(concat_feat + attn_feat)
        # 5. 拆分回3个128维特征（复用原模型双分支逻辑）
        embed_calib = calibrated_feat[..., :128]
        pos_calib = calibrated_feat[..., 128:256]
        blosum_calib = calibrated_feat[..., 256:]
        return embed_calib, pos_calib, blosum_calib, attn_weights


# ===================== 修改Net：新增特征校准模块，复用原双分支逻辑 =====================
class Net(nn.Module):
    def __init__(self, num_classes=2):
        ...
        self.branch_attn = nn.MultiheadAttention(
            embed_dim=20,  # 原模型卷积输出通道数，完全适配
            num_heads=5,
            batch_first=True,
            dropout=0.0
        ).to(device)
        self.attn_scale = nn.Parameter(torch.ones(1) * 0.1)  # 弱化注意力影响

        # 新增：自适应池化层（用于动态长度对齐）
        self.adaptive_pool = AdaptiveAvgPool1d(1)  # 动态适配任意目标长度
    ...
    def forward(self, x, pos_embed, HF):
        ...
        fused = q + attn_out  # 残差连接，复用原特征
        fused = fused.permute(0, 2, 1)  # 转回原格式

        # ===================== 完全复用原模型后续逻辑 =====================
        output = torch.cat([output1, fused], 2)  # 替换原output2为融合后的特征
