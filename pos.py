...
def data_construct(seqs, labels,file, train):#将氨基酸序列转化为模型所需要的输入
    ...
    if(file=='Ind.csv'):
        ...
        # 使用pickle保存
        with open('pos_test_loader_config.pkl', 'wb') as f:
...
class Net4(nn.Module):
    ...
    def forward(self, x, pos_embed, HF):
        ...
        output = output1#(batch,channels,seq)
...
def train_test(train_iter, test_iter):
    ...
    for epoch in range(EPOCH):
        ...
        if (epoch+1)%50==0:
            torch.save(net,"pos_mlp.pth".format(epoch+1))
    #writer.close()
...
if __name__ == '__main__':
    ...
    ind_iter = load_Ind_data('main.csv')
